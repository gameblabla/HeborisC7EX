...
void viewBack(void) {
	...
	if(bgfadesw) {
		...
		if(bgfade_cnt < 14){
			...
			if(bgfade_cnt == 6) {
				backno++;
				if((overcount != 0) || (stat_game[0] == 23)) backno = 0;
...
void viewScoreSmall(void) {
	...
	if( ((stat_game[i] != 2) || ((stat_game[i] == 2) && (statc[i * 10 + 2] != 0))) && (stat_game[i] != 0) && ((stat_game[i] !=1) || ((stat_game[i] ==1) && (statc[i * 10 + 1] != 2))) ){
				...
				if(gflash[i] > 108) {
					...
					// 非DEVIL
						...
						if( (grade[i] > 0) || (!devil_minus[i]) ) {
							...
							else if(stat_game[i] != 2)  //1（DOOMのみ）
...
// QUALIFY
			if((gameMode[i] == 1) &&(examination[i]) && (stat_game[i] != 30)) {
...
// TOMOYO
			if( gameMode[i] == 6) {
				...
				if(!hide_tomoyoinf && (stat_game[i] >= 3) && (stat_game[i] < 18) || (stat_game[i] == 19) || (stat_game[i] == 20)) {
					// NEXTC
...
// ORIGINAL
			...
			/* メダル表示 */
			// 条件分岐を簡略化
			if((gameMode[i] != 0)&&(gameMode[i] != 4) && (gameMode[i] <= 5)&&(stat_game[i]!=2)) {
...
void viewTime(void) {
	...
	//			color2 = color;
		...
		if(debug){		// 数値チェック用スペース	バグチェックにどうぞ。
			sprintf(string[0], "STAT:%2d", stat_game[i] );
	...
	if((playback) && (!demo) && (gameMode[0] != 6) && (gameMode[0] != 8) && (gameMode[0] != 4)){
		if((onRecord[0]) || (ending[0]) || (stat_game[0] == 7) || (stat_game[0] == 14)){
			if(dispLinfo){
...
void viewField(void) {
	...
	for(i = 0; i <= mp; i++) {
		...
		if((stat_game[i] == 18) && (statc[i * 10 + 1] == 2)){
				for(j = 0; j < 20; j++)
					for(k = 0; k < 10; k++) {
		...
		//通常
		} else if((!pause_game[i]) || (debug) || ((gameMode[i] == 5) && (pause_game[i]))) {
			for(b = 0; b < 2; b++)
							...
							//	if(hidden[i] == 8) color = 20;
								...
								if( (isblind[i]) || (blockflash != 2) && ( (!isxray[i]) || ( (xray_counter[i] % p_xray_frame == k) && (!xray_drawblockframe) ) ) ) {
									...
									if( ((fi != 8)&&(fi != 9) && (add == 0)) || (isblind[i])) {
										...
										if( (fldt3 == -1) || ((fldt3 != 0) && (fldt3 >= 8) && (m_roll_blockframe)) || ((isblind[i]) && (fldt3 != 0)) || (fldt3 < -1) || ((gameMode[i] == 5) && (pause_game[i]))) {
											// 死んだとき枠を消す #1.60c7k9
											// COLOR中は枠を描かない#1.60c7p9ex
											if(( !color_flag[i] && ((!dead_blockframe) || (gameMode[i] == 5) || (stat_game[i] != 7)) )&& (heboGB[i]==0) || (isblind[i])) {
												// フィールド端では枠の一部分を表示しない #1.60c7s5
												if((!isdark[i]) || ((gameMode[i] == 5) && (pause_game[i]))){
													// 新・枠線処理
...
void viewPreview(void){
	...
	for(i = 0; i <= mp; i++) {
		...
		if( (hnext[i] >= 1) && (stat_game[i] != 0) && ((stat_game[i] != 2) || ((stat_game[i] == 2) && (statc[i * 10 + 2] != 0))) ) {
...
void viewFldFrame(int uponly,int i) {
	int j, gm[2],waiting[2],ismission[2];
		...
		if(gameMode[i] == 10)//ORI
			...
			if((stat_game[i] == 31) && (statc[i * 10] < 120)){
				gm[i] = 1 + (5 * (count%4 <= 1));
			} else if((stat_game[i] == 25) || (stat_game[i] == 33) || (isallclear[i]) || ((isreflect[i]) && (gameMode[i] != 4) && (!isfever[i]))){
				gm[i] = 6;		//サポートアイテム（明るい青）
			} else if( (doexchg[i]) || ((stat_game[i] == 29) && (!statc[(1-i) * 10 + 3])) || (isFreefall[i]) || ((stat_game[i] == 26) && (statc[i * 10 + 2])) ){
				gm[i] = 0;		//その他（緑）
			...
			(isrotatelock[i]) || (isholdlock[i]) || (istimestop[i]) || (ishidenext[i]) || (ismagnetblock[i]) ||
			 ...
			 (dolaser[i]) || (stat_game[i] == 27) || (donega[i])  || (stat_game[i] == 26) || (stat_game[i] == 28) || (doshotgun[i]) ||
			 (item_nblk[0 + i * 6] == fldihardno) || (item[i] == fldihardno) || (isbanana[i]) || (do180field[i])  || (stat_game[i] == 34)||
			 (isdouble[i])||(docopyfld[i])||(isfakenext[i])||(item_monochrome[i])||(ismiss[i])||(stat_game[i] == 29) && (statc[(1-i) * 10 + 3]) ){
				gm[i] = 2;		//攻撃アイテム（赤）
			} else if(isfever[i]){
				gm[i] = 3;		//フィーバー（黄）
			}else if(gameMode[i] == 3){
		...
		if((stat_game[i] == 0) || ((stat_game[i] == 2) && (statc[i * 10 + 2] == 0)) || ((stat_game[i] ==1) && (statc[i * 10 + 1] == 2))){
			gm[i] = 5;
			waiting[i] = 1;
		}
...
void drawBlock(int bx1, int by1, int kind, int rotate, int color, int bright, int offset, int flash, int player, int use3D) {
	...
	for(j = ((!use3D) || (tmp) || (flash == 999) || (heboGB[player]>0)); j < 3; j++){
		...
		if((j == 2) && ((flash != 10) || (stat_game[player] == 5))){
			// 周り枠 #1.60c7o5
